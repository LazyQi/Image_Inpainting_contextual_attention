...
parser.add_argument('--input_dirimg', default='examples/imagenet/ILSVRC2012_val_00000019.JPEG', type=str,
                    help='The input folder path for multi-images')
parser.add_argument('--output_dirmask', default='examples/wqmask/', type=str,
                    help='The output file path of mask.')
parser.add_argument('--output_dirmasked', default='examples/maskedimg', type=str,
...
def random_mask_rect(img_path, config, bsave=True):
    ...
    image = cv2.resize(img_data, (256, 256))
...
def load_mask(img_path, config, bsave=False):
    ...
    cv2.imwrite("examples/maskedimg/rawimg.jpg", img)
    shape = img.shape
